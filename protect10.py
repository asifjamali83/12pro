...
kc = LINETCR.LINE()#LULY 4
kc.login(token='EqicsCjcHpVC4XlNP8O2.ZWXwD1cn0eMz0FrM4e6YGG.vKDlz0NSxJ6s62UBLEmGsad9QyxkYyi42qN5KehZDiU=')
kc.loginResult()

ks = LINETCR.LINE()#LULY 5
ks.login(token='EqVKZCEeX6gMZGpVphd3.Ojugj6pI675koFyInIg/eW.i0OHjgJ1wATm4k4ZgrkIjJ5EVZE3A02aXqu44SF8p/s=')
ks.loginResult()
...
#---------------------
        if op.type == 17:
          if wait["Sambutan"] == True:
           ...
           image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
           cl.sendImageWithURL(op.param1,image)
           random.choice(KAC).sendText(op.param1,"Halo " + cl.getContact(op.param2).displayName  + "\nJones " +  wait["welmsg"] + " " + str(ginfo.name) + " ☜" + "\nBudayakan Baca Note !!! yah Ka 😊\nSemoga Betah Kk 😘")
        ...
        if op.type == 15:
          if wait["Sambutan"] == True:
           ...
           image = "http://dl.profile.line-cdn.net/" + contact.pictureStatus
           cl.sendImageWithURL(op.param1,image)
           random.choice(KAC).sendText(op.param1,"Byee " + cl.getContact(op.param2).displayName + "\n" + wait["welmsg2"] + "\nHati - hati dijalan yah ka 😘")
